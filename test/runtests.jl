...
@testset "SimpleDirectedHypergraphs paths/distance         " begin
    ...
    @test init_state.reached_vs == BitVector(falses(nhv(dh2)))
    @test init_state.marked_hes == BitVector(falses(nhe(dh2)))
    @test init_state.removed_hes == BitVector(falses(nhe(dh2)))
    ...
    fr = forward_reachable(dh2, 1, init_state)
    @test fr[1] == Set{Int}(1:5)
    @test fr[2] == Set{Int}(1:9)
    fr = forward_reachable(dh2, 5, init_state)
    @test fr[1] == Set{Int}(1:5)
    @test fr[2] == Set{Int}(1:9)

    # Back-tracing from vertices to vertices/hyperedges
    bt = backward_traceable(dh2, 1, init_state)
    ...
    init_state = initialize_dihyperpath_state(dh3, w3)

    # Traversing forward from vertices to vertices/hyperedges
    fr = forward_reachable(dh3, 1, init_state)
    @test fr[1] == Set{Int}(1:9)
    @test fr[2] == Set{Int}(1:11)
    fr = forward_reachable(dh3, 6, init_state)
    @test fr[1] == Set{Int}([6, 8, 9])
    @test fr[2] == Set{Int}([9, 11])
    fr = forward_reachable(dh3, 9, init_state)
    @test fr[1] == Set{Int}(9)
    @test fr[2] == Set{Int}()

    # Back-tracing from vertices to vertices/hyperedges
    bt = backward_traceable(dh3, 1, init_state)
    @test bt[1] == Set{Int}(1)
    @test bt[2] == Set{Int}()
    bt = backward_traceable(dh3, 6, init_state)
    @test bt[1] == Set{Int}([6, 1])
    @test bt[2] == Set{Int}(3)
    bt = backward_traceable(dh3, 9, init_state)
